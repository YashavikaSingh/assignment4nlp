...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        ...
        # Now generate SQL queries
        for encoder_input, encoder_mask, _, _, initial_decoder_inputs in tqdm(dev_loader, desc="Generating"):
            encoder_input = encoder_input.to(DEVICE)
            encoder_mask = encoder_mask.to(DEVICE)
            initial_decoder_inputs = initial_decoder_inputs.to(DEVICE)
            
            # Generate SQL queries using greedy decoding
            ...
            all_generated_queries.extend(generated_sql)
    ...
    # Return order: loss, record_f1, record_em, sql_em, error_rate (to match train function)
